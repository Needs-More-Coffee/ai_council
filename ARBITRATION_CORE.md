...
The wrapper reads the Steward's flags. If the Steward produced specific flags, round one order is calculated by flag count with ties broken by seat order. If the Steward produced no flags, all voices are treated as equally flagged and round one proceeds in full seat order.
