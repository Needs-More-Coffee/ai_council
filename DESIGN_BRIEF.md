...
Flags are positional strings that identify which voices an entry is directing attention toward. Seat order — declared by the wrapper at session open and fixed for the duration of the session — defines the positional map. An X in a position means that voice is flagged. An underscore means it is not. A single flag string can address any combination of voices simultaneously.

For example, in a seven voice session with seat order set at open, a flag of (X__X_X_) signals voices in positions one, four, and six.
